...
class WSGILogger(object):
    ...
    def __call__(self, environ, start_response):
        ...
        content_length = (
            content_lengths[0] if content_lengths else len(b"".join(retval))
        )
        msg = self.formatter(
            status_codes[0],
            environ,
...
class ApacheFormatters(object):
    ...
    def format_with_response_time(*args, **kwargs):
        ...
        return ApacheFormatters.format_NCSA_log(*args, **kwargs) + " {0}/{1}".format(
            int(rt_us / 1000000), rt_us % 1000000
        )
