...
class Pill(object):
    ...
    def __init__(self, prefix=None, debug=False, record_format=Format.JSON):
        ...
        self._filename_re = re.compile(
            r".*\..*_(?P<index>\d+).{0}".format(record_format)
        )
    ...
    def record(self, services="*", operations="*"):
        ...
        for service in services.split(","):
            for operation in operations.split(","):
                event = "after-call.{0}.{1}".format(service.strip(), operation.strip())
    ...
    def _record_data(self, http_response, parsed, model, **kwargs):
        ...
        self.save_response(
            service_name, operation_name, parsed, http_response.status_code
        )
    ...
    def get_next_file_path(self, service, operation):
        ...
        while not next_file:
            ...
            if next_file:
                ...
                raise IOError(
                    "response file ({0}.[{1}]) not found".format(
    ...
    def save_response(self, service, operation, response_data, http_response=200):
        ...
        data = {"status_code": http_response, "data": response_data}
    ...
    def load_response(self, service, operation):
        ...
        return (FakeHttpResponse(response_data["status_code"]), response_data["data"])
