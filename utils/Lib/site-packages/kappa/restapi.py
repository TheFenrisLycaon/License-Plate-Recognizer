...
class RestApi(object):
        ...
        if self._resources is None:
            try:
                response = self._apigateway_client.call(
                    "get_resources", restApiId=self.api_id
                )
                LOG.debug(response)
                self._resources = {}
                for item in response["items"]:
    ...
    def create_restapi(self):
        if not self.api_exists():
            ...
            try:
                response = self._apigateway_client.call(
                    "create_rest_api", name=self.api_name, description=self.description
                )
                LOG.debug(response)
            except Exception:
    ...
    def create_deployment(self):
        LOG.info(
            "creating a deployment for %s to stage: %s",
            self.api_name,
    ...
    def delete(self):
        ...
        try:
            response = self._apigateway_client.call(
                "delete_resource", restApiId=self.api_id, resourceId=self.resource_id
            )
            LOG.debug(response)
        except ClientError:
