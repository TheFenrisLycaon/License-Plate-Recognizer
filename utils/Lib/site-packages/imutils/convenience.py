...
def build_montages(image_list, image_shape, montage_shape):
    ...
    montage_image = np.zeros(
        shape=(
            image_shape[1] * (montage_shape[1]),
            image_shape[0] * montage_shape[0],
            3,
        ),
        dtype=np.uint8,
    )
    ...
    for img in image_list:
        ...
        montage_image[
            cursor_pos[1] : cursor_pos[1] + image_shape[1],
            cursor_pos[0] : cursor_pos[0] + image_shape[0],
        ] = img
...
def adjust_brightness_contrast(image, brightness=0.0, contrast=0.0):
    """
    ...
    return cv2.addWeighted(
        image, 1 + float(contrast) / 100.0, image, beta, float(brightness)
    )
